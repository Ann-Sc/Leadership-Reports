...
#Cleaning data
field_data <- data %>%
  filter(Name...1 != "Renewal") %>% #filtering out Renewal Team
...
The code chunk below creates a new column that assigns a sub - category to each activity. New and unassigned activities can be added to this chunk.
...
categorized_data <- field_data %>%
  mutate(sub_category = case_when(
    ...
    Name...2 %in% c("Number of counseling sessions provided", "Number of people given counseling") ~ "Counseling Sessions",
...
small_brand <- c("#EF5A4A","#55373C","#0E8BA4")
```
...
rrr_plot <- plot_ly(plot_data, labels = ~Category, values = ~Quantity, type = 'pie', marker = list(colors = brand),
...
#to save interactive plot as html
...
total_activity_plot <- plot_ly(plot_data, labels = ~Name...2, values = ~Quantity, 
...
#REACH data
reach_data <- plot_data %>%
  filter(sub_category %in% c("Pre E Encounter","Gospel Conversation","Formal Gospel Encounter"))

#Plot
reach_plot <- plot_ly(reach_data, labels = ~sub_category, values = ~Quantity, type = 'pie', textinfo = 'label+percent', 
        marker = list(colors = brand), showlegend = FALSE)
...
#RENEW data
renew_data <- plot_data %>%
  filter(sub_category %in% c("Discipleship Encounter","Train & Support local leaders","Church Plant Activity"))

#Plot
renew_plot <- plot_ly(renew_data, labels = ~sub_category, values = ~Quantity, type = 'pie', textinfo = 'label+percent', 
        marker = list(colors = brand), showlegend = FALSE)
...
restore_plot <- plot_ly(restore_data, labels = ~sub_category, values = ~Quantity, type = 'pie', textinfo = 'label+percent', 
                     marker = list(colors = c("#55373C","#0E8BA4")), showlegend = FALSE,insidetextorientation = 'horizontal')
...
## Data and Visualisations including Renewal Team
